...
export default defineConfig({
  ...
  define: {
    ...
    __MDE_PERF_BUDGETS__: JSON.stringify({
      ...
      suggestionFilterP95: Number(process.env.MDE_WEB_SUGGESTION_FILTER_P95_BUDGET_MS ?? 8),
      ...
      pluginScale50EditP95: Number(process.env.MDE_WEB_PLUGIN_SCALE_50_EDIT_P95_BUDGET_MS ?? 20),
      pluginScale50Install: Number(process.env.MDE_WEB_PLUGIN_SCALE_50_INSTALL_BUDGET_MS ?? 35),
    }),
  },
  publicDir: false,
  plugins: [
    {
      ...
      configureServer(server) {
        ...
        server.middlewares.use('/__mde_perf_media_report', (request, response, next) => {
          if (request.method !== 'POST') { next(); return; }
          const chunks: Buffer[] = [];
          request.on('data', (chunk) => chunks.push(Buffer.from(chunk)));
          request.on('end', async () => {
            const directory = `${here}../target/performance`;
            await mkdir(directory, { recursive: true });
            await writeFile(`${directory}/web-media-metrics.json`, Buffer.concat(chunks));
            response.writeHead(204).end();
          });
        });
      },
  ...
  build: {
    ...
    lib: {
      entry: {
        ...
        'extensions/suggestions': `${here}extensions/suggestions.ts`,
        'extensions/productivity': `${here}extensions/productivity.ts`,
        'extensions/journal-attachments': `${here}extensions/journal-attachments.ts`,
        'extensions/composer': `${here}extensions/composer.ts`,
        'extensions/examples': `${here}extensions/examples.ts`,
      },
      formats: ['es'],
